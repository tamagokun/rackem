...
class Utils
{
    ...
    public static function parse_php_form_data($post, $files)
    {
        // fix structure of $_FILES super global
        foreach ($files as $k => $v) {
            $arr = array();
            foreach ($v as $prop => $propval) {
                if (is_array($propval)) {
                    array_walk_recursive($propval, function(&$item, $key, $value) use($prop) {
                        $item = array($prop => $item);
                    }, $v);
                    $arr = array_replace_recursive($arr, $propval);
                } else {
                    $arr[$prop] = $propval;
                }
            }
        ...
        return self::array_merge_recursive($post, $files);
    }

    private static function normalize_php_files($arr)
    {
        $files = array();
        $keys = array('name', 'type', 'tmp_name', 'error');
        if ($keys == array_keys($arr)) {
            // we have our files array, lets fix it.
            foreach ($arr as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $file_key => $v) {
            ...
            foreach ($arr as $k => $v) {
                if (is_array($v) && $keys == array_keys($arr)) {
                    $arr[$k] = $this->normalize_php_files($v);
                }
            }
        }
    }
